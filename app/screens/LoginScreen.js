...
export default class LoginScreen extends Component {
  ...
  async httpTest() {
    ...
    let anos = await anoService.get();
    console.log('[RESULT] anos', anos);

    let disciplina = await anoService.one(1).all('disciplinas').one(1).get();
    console.log('[RESULT] disciplina 1', disciplina);

    let ano1 = await anoService.one(1).get();
    let disciplinas = await ano1.link.disciplinas.get();
    console.log('[RESULT] disciplinas do ano 1', disciplinas);

    let anosByProfessor = await anoService.findByProfessor(5);
    console.log('[RESULT] anos do professor 5', anosByProfessor);

    let responsabilidades = await disciplina.link.responsabilidades.get();
